...
- **Run CRD** (`api/v1alpha1/run_types.go`): the central state machine. Phase: Pending → Scheduled → Running → Succeeded/Failed. Spec holds runtime name, commands, env, timeout. Status holds phase, assignedPod, message, timestamps. CRD group: `kruntimes.kruntimes.com/v1alpha1`. Short name: `rn`. Scheduler treats empty phase as equivalent to Pending (the kubebuilder default on status subresource doesn't apply on Create).
