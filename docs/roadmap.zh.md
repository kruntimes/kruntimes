...
- [x] Release/package hygiene：去掉已发布 image package 名字里冗余的
  ...
  的不一致。
- [ ] Run input semantics：统一并稳定 `inline`、`entrypoint`、`args` 在 API、
...
- [ ] CLI baseline：增加 `krt version`，方便用户和维护者确认当前 CLI version、
  commit 和 build timestamp。
- [ ] Benchmark correctness：诊断为什么 `latency.complete` 明显高于手动创建单个
