...
- [ ] Agent sandbox 所需的 Function-mode Runs：定义 `Run.spec.mode=Function`
  语义，让一个 Run 可以 reserve 预热 Runtime Pod，向 runtimed/runtime-server 注册
  callable function，保持 ready 状态以支持多次低延迟 invocation，并在删除或 idle
  timeout 时释放 reservation。这个能力应该走 dataplane invoke path，而不是为每次
  invocation 创建 Kubernetes object。
- [ ] Runtime gateway invoke path：为每个 Runtime 创建一个 gateway Service，把这个
  ...
  同步读取 Kubernetes API。
- [ ] Function-mode API cleanup：重新审视现有 `Run.spec.handler` 字段。这个字段是在
  function mode 还没有完整设计时加入的；定义 `Run.spec.mode=Function` 之后，要么把
  `handler` 迁移到新的 function registration model 中，要么在 API 稳定前移除。
...
- [ ] Agent sandbox SDKs：为 agent 开发者提供 first-class SDK，优先支持 Python 和
  Go。SDK 应隐藏 Kubernetes object 创建、readiness polling、gateway discovery、本地
  开发的 port-forward fallback、in-cluster direct URLs、invoke calls、有界
  outputs/artifacts、timeouts、幂等操作 retries、auto-cleanup、disconnect/reconnect
  行为和 typed errors。
...
- [ ] Agent framework integration：为 agent frameworks 和 MCP-style tool servers 设计
  一层轻量集成，使 tool call 可以 acquire 或 reuse function-mode Run，通过 gateway
  invoke，返回结构化结果，并按照 agent session policy cleanup 或 preserve sandbox。
