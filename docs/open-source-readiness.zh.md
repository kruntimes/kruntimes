...
- [x] 梳理并统一 `args`、`source.inline` 和 `entrypoint` 在内置 Runtime、CLI、API
  文档和示例中的语义。
...
- [x] 发布 Helm OCI chart。
- [x] 使用 Hugo 整理项目文档网站，并通过 GitHub Pages 发布。
...
- [x] 增加性能基准：调度延迟、吞吐量、Runtime 容量和控制面负载。
- [x] 校准 README roadmap，只将有实现、测试和可用安装路径的能力标为完成。

## P3: 公开后的产品验证

- [x] 发布对比指南，覆盖 kruntimes vs Knative、Argo Workflows、Tekton、
  Volcano，以及基于 Deployment 的 worker queue。
- [x] 增加 “when to use / when not to use” 指南，明确 kruntimes 是 warm
  execution substrate，不是完整 serverless platform、workflow engine、batch
  scheduler replacement 或 hostile-code sandbox。
- [ ] 招募 5-8 个来自 platform、CI 和 AI agent infrastructure 团队的目标用户，
  验证他们是否真实遇到 Pod cold start、burst throughput 或无法接管底层
  infrastructure 的约束。使用 [目标用户验证 Playbook](user-validation.md) 执行并记录
  这个流程。
- [x] 发布三个端到端 demo：低延迟 Bash/Python Run、burst short-task execution、
  custom Bash Runtime image。
- [x] 定义并跟踪公开后的 adoption signals：用户能在两分钟内解释项目价值，
  至少两个 design partners 用真实 workload 试用，至少一个非 maintainer 跑通
  quick start。
- [ ] 验证第一个 primary wedge。当前假设是 AI agent tools 和 trusted internal
  code-execution sandboxes，CI micro-steps 和 automation tasks 作为次级场景。使用
  [目标用户验证 Playbook](user-validation.md) 中的判断规则。
