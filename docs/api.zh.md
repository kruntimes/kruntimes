...
并推导 step 与 job status，再将 settled jobs 聚合为 WorkflowRun 终态。Reusable job
calls、Action expansion、output propagation 和 cancellation propagation 仍在 roadmap 中。
...
| Field | Description |
| --- | --- |
| `status.phase` | `Pending`、`Running`、`Succeeded`、`Failed` 或 `Cancelled`。所有 jobs settled 后，任一 job failed 则 controller 设置为 `Failed`，否则设置为 `Succeeded`。Cancellation handling 仍在 roadmap 中。 |
