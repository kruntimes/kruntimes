...
dependency failure propagation、WorkflowRun terminal aggregation 和 cancellation propagation
仍在 roadmap 中。
...
`Skipped` 表示 job 被 failed 或 skipped dependency 阻塞；将 dependency 状态传播为该 phase
是下一项 controller task。
