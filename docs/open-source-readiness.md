...
时无法清理。现在由长期运行的 controller 从 Run status 中读取持久化的 store 配置快照，
并在 controller reconcile 进程内执行幂等清理，避免依赖额外 cleanup Pod 被调度。
...
- [ ] 建立 SemVer、CHANGELOG 和 release notes 流程。
- [ ] 发布 scheduler/controller/runtimed/runtime 镜像。
- [ ] 发布 Helm OCI chart。
- [ ] 发布 `krt` 多平台二进制、checksum 和 provenance。
- [ ] 增加 Kubernetes/Helm/Go/Python 兼容性矩阵。
- [ ] 增加安装、升级、卸载、故障排查和备份恢复文档。
- [ ] 增加自定义 Runtime 开发指南和协议兼容性约定。
