...
- artifacts 访问需要 Run read permission，以及选定 artifact-store backend 所定义的权限；
...
Workflow 和日志访问，不授予 Secrets 或 workload mutation verb；日志读取所需的
`pods/portforward` `create` subresource permission 是唯一例外：
