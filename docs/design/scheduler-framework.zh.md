...
- 让同一个 planning cycle 中较早 assignment 的结果影响后续 Runs，而不把临时状态暴露到 Kubernetes API。
...
scheduler 在每个 planning cycle 中从一个 key 处理有界的 eligible Runs。集合大小和 planning time budget
是实现配置，而不是 Run API fields。达到预算后，其余 eligible Runs 留给下一个 cycle。确定性的基础排序使用 creation timestamp
和 UID；未来经过 review 的 priority policy 可以替换 queue ordering，但必须保留 aging 或 fairness rules。

变化只是 keyed work queue 的输入，而不是 placement decision。举例来说，若同一 Runtime 的 `run-a` 和
`run-b` 都是 Pending，两个 event 会 enqueue 同一个 `(namespace, runtime)` key；任一 event 都不会独立为
对应 Run 做 assignment。scheduler dequeue 这个 key 后，会对这两个 Run 和 Runtime Pods 获取同一个 snapshot，
再基于同一份 reservation state 规划各自独立的 assignment。
...
4. **Score**：按 preferred affinity、available capacity 和 least loaded 对 eligible Pods 打分。Pod name 用于
   stable tie breaking。
5. **Reserve**：在内存 planning state 中记录选中的 Pod 并消耗 capacity。同一 planning cycle 的后续 Runs 可以看到
...
每个 reservation 都独立处理：scheduler 分别尝试 bind 每个 reserved Run；一条 Run 失败不会回滚其它已经
成功 bind 的 Run。与 Kubernetes 一样，reservation 是 assumed 的内存 placement：它让后续 scheduling
decision 在 status patch 完成前看到 capacity consumption。reservations 不会以 annotation、capacity counter 或
user-visible status 字段持久化。restart 后，下一个 snapshot 根据 assigned active Runs 重建 capacity，因此不需要
独立的 reservation recovery protocol。

## 高可用

高可用与 queue 和 affinity 语义分离。初版 implementation 要求整个集群只有一个 active scheduler
planner。Helm deployment 已启用 controller-manager leader election，因此 standby replicas 不会消费 queue
keys，也不会写入 Run assignments。

leader failover 后，新 active planner 从空的 in-memory queues 和 reservations 开始：它从 Pending Runs
重建相关 queue，并在第一次 snapshot 中从 assigned active Runs 重建 capacity。尚未完成 status patch 的
reservation 会因此消失；已成功 patch 的 Run 则会作为 actual assignment 被观察到。未来若要 scheduler
sharding，需要独立的 ownership design；本文不隐含这一能力。
...
- **planned target**：同一 planning cycle 中较早 Run 的 reservation。
...
出现 matching 的 active 或 planned target。
...
- **Reserve/Bind**：reservation 会让选中的 Runtime Pod 和已消耗的 capacity 在同一个 planning cycle 中
  对后续 decision 可见，再分别 bind 各个 Run。

## 可观测性

实现应保留现有 scheduling latency 与 result metrics，再增加 queue activation、planning set size、planning duration、
...
4. 实现 planned-target matching 和 Run 间亲和性 bootstrap，并增加 integration 与 E2E coverage。
