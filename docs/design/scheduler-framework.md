# Scheduler Framework and Batch Planning
...
It does not add a public API in this design slice. Any `Run.spec.priority` or
explicit group-scheduling API requires a separate reviewed API design.
...
- Let assignments made earlier in a planning batch influence later assignments
...
- Provide an extension point for priority, fairness, and an eventual explicit
...
- Implicit all-or-nothing placement for arbitrary dependency cycles.
...
The scheduler leader drains a bounded batch from one key. The batch size and
planning time budget are implementation configuration, not Run API fields.
...
5. **Reserve**: record the chosen Pod and consume capacity in the in-memory
   planning state. Subsequent Runs in the same batch observe this tentative
   assignment.
...
The scheduler commits only the final Run assignments. Reservations are never
persisted as annotations, capacity counters, or user-visible status fields.
...
- a **planned target**, an earlier Run reservation in the same batch.
...
This is intentionally analogous to Kubernetes' self-affinity bootstrap rule:
the first member may be scheduled when no matching member exists, provided it
matches the term itself. It prevents a homogeneous affinity cohort from waiting
forever while preserving the meaning of a required constraint.

This rule does **not** solve arbitrary mutual dependency cycles. If Run A
requires labels only Run B has and Run B requires labels only Run A has, neither
is a self-affinity seed. They remain unschedulable with an affinity reason. A
future workload that requires strict all-or-nothing placement must use an
explicit, reviewed scheduling-group API rather than rely on incidental queue
order or inferred Workflow structure.
...
- **Reserve/Bind**: an eventual explicit scheduling group can atomically plan
  a known group before any of its Runs are bound.

An explicit group feature must define membership, minimum/required cardinality,
timeout, cancellation, partial success, and observability. It is not implied by
Run affinity and is not part of this proposal's API.

## Observability

The implementation should retain scheduling latency and result metrics, then
add bounded labels/counters for queue activation, batch size, planning duration,
...
4. Implement planned-target matching and self-affinity bootstrap with
   integration and E2E coverage.
5. Add priority only after a separate API and fairness design review.
6. Design an explicit scheduling-group API only when a Workflow or batch demo
   proves the need for all-or-nothing placement.
