...
function 支持对 custom Runtime 是 opt-in：只支持 task 的 Runtime 只需实现并注册 `Runtime`。
...
基础 `executor.v1.Runtime` service 保持 task-only。支持 function 的 Runtime Server 会在同一
...
配置失败：Run 不能回退到 task mode，而是依照不兼容 Runtime 的正常 terminal 或 retry policy 处理。
