...
`Run.spec.handler` 现在作为早期 FaaS 相关 top-level 字段存在：
...
选择具体 callable entrypoint。问题在于它的位置。top-level `handler` 和 task-only 的
`entrypoint`、`args` 并列，会让 execution model 更难理解。

目标 API 应把 handler 放到 function mode 下：
...
API 设计应在稳定前把当前 top-level `Run.spec.handler` 迁移或替换为
`Run.spec.mode.function.handler`。Task mode 应把 `entrypoint` 和 `args` 放在
`mode.task` 下，function mode 应把 `handler` 放在 `mode.function` 下。
...
1. 增加 mutually exclusive `spec.mode.task` 和 `spec.mode.function` 的 API 设计和
   validation。
2. 将 top-level `Run.spec.handler` 迁移或替换为 `Run.spec.mode.function.handler`。
